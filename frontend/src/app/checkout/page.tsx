        ...
        <div>
          ...
          {!showForm ? (
            ...
            <form onSubmit={handleCreateAddress} className="bg-gray-50 p-4 rounded-lg border space-y-3 animate-in fade-in slide-in-from-top-2">
              <input required placeholder={t("street")} className="w-full p-2 border rounded" 
                onChange={(e) => setNewAddress({...newAddress, street: e.target.value})} />
              <div className="grid grid-cols-2 gap-2">
                <input required placeholder={t("city")} className="w-full p-2 border rounded" 
                  onChange={(e) => setNewAddress({...newAddress, city: e.target.value})} />
                <input required placeholder={t("state")} className="w-full p-2 border rounded" 
                  onChange={(e) => setNewAddress({...newAddress, state: e.target.value})} />
              </div>
              <div className="grid grid-cols-2 gap-2">
                <input required placeholder={t("zip")} className="w-full p-2 border rounded" 
                  onChange={(e) => setNewAddress({...newAddress, postalCode: e.target.value})} />
                <input required placeholder={t("country")} className="w-full p-2 border rounded" 
                  onChange={(e) => setNewAddress({...newAddress, country: e.target.value})} />
              </div>
              <div className="flex gap-2 justify-end">
                <button type="button" onClick={() => setShowForm(false)} className="text-gray-500 text-sm px-3">{tc("cancel")}</button>
                <button disabled={processing} className="bg-black text-white px-4 py-2 rounded text-sm font-bold">{tc("save")}</button>
              </div>
            </form>
          )}
        </div>
